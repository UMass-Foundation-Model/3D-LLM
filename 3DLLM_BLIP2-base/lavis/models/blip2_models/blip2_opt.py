...
class Blip2OPT(Blip2Base):
    ...
    def forward(self, samples):
        image = samples["image"]
        image_embeds = samples["pc_feat"]

        pc = samples["pc"].long()
        all_pcs = torch.zeros((image_embeds.shape))
        ...
        image_embeds = torch.cat([image_embeds, all_pcs], 1)
    ...
    def predict_answers(
        ...
        **kwargs,
        ...
        input_tokens = self.opt_tokenizer(text, padding="longest", return_tensors="pt").to(image.device)
