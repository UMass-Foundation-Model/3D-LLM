...
if __name__ == '__main__':
    ...
    dataset_dir = "/gpfs/u/home/LMCG/LMCGnngn/scratch-shared/"

    scene_lists = sorted(os.listdir(dataset_dir + "mp3d_train_rgb_depth_pose/"))
    for scene in tqdm(scene_lists):
        if True:
            try:
                os.mkdir(os.path.join(dataset_dir, "mp3d_train_nps_1024", scene))
            except:
                pass

            for file in os.listdir(os.path.join(dataset_dir, "mp3d_train_masks", scene)):
              
                try:
                    INPUT_IMAGE_PATH = os.path.join(dataset_dir, "mp3d_train_rgb_depth_pose", scene, file.replace(".pt", ".png"))
                    SEMIGLOBAL_FEAT_SAVE_FILE = os.path.join(dataset_dir, "mp3d_train_nps_1024", scene, file)
                    if os.path.isfile(SEMIGLOBAL_FEAT_SAVE_FILE):
                        continue

                    raw_image = cv2.imread(INPUT_IMAGE_PATH)
                    raw_image = cv2.resize(raw_image, (512, 512))
                    #image = torch.tensor(raw_image[:242, :242]).permute(2, 0, 1).unsqueeze(0).to(device)
        # print(image.shape)
                    image = torch.tensor(raw_image).to(device)
            
                    #visual_encoder = create_eva_vit_g(242).to(device)
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        # tqdm.write(f"Image dims: {_img.shape} \n")
                        # tqdm.write(f"Image feature dims1: {imgfeat.shape} \n")
                        
                        #tqdm.write(f"Saving to {GLOBAL_FEAT_SAVE_FILE} \n")
                        #torch.save(imgfeat.detach().cpu().half(), GLOBAL_FEAT_SAVE_FILE)
                    # print(OPENCLIP_MODEL)
                    # exit()
                    ...
                    MASK_LOAD_FILE = os.path.join(dataset_dir, "mp3d_train_masks", scene, file)
                    ...
                    #mask = mask[:, :, :242, :242]
                    #mask = torch.nn.functional.interpolate(mask, [224, 224], mode="nearest")
                ...
                # viz[x0:x1, y0:y1, 0] = 1.0
                        ...
                        #del curmask, img_roi, roifeat, nonzero_inds, bbox, _sim
            # """
            ...
            #print(softmax_scores)
                    for _roiidx in range(retained.shape[0]):
                        ...
                        # print (retained_nonzero_inds[_roiidx][:, 0])
