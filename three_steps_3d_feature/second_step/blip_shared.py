...
if __name__ == '__main__':
    ...
    for scene in tqdm(os.listdir(dataset_dir + "mp3d_test_rgb_depth_pose")):
        try:
            if scene + ".pt" in os.listdir(dataset_dir):
               continue
    
            try:
                os.mkdir(os.path.join(dataset_dir, "mp3d_test_nps", scene))
            except:
                pass
    
            for file in os.listdir(os.path.join(dataset_dir, "mp3d_test_masks", scene)):
                INPUT_IMAGE_PATH = os.path.join(dataset_dir, "mp3d_test_rgb_depth_pose", scene, file.replace(".pt", ".png"))
                SEMIGLOBAL_FEAT_SAVE_FILE = os.path.join(dataset_dir, "mp3d_test_nps", scene,  file)
                ...
                MASK_LOAD_FILE = os.path.join(dataset_dir, "mp3d_test_masks", scene, file)
            ...
            #np.save(os.path.join("nps/%s/whole.pt"%file.replace), np.array(output.cpu().detach().numpy()))
            ...
            #iimg_roi = preprocess(img_roi).unsqueeze(0).cuda()
