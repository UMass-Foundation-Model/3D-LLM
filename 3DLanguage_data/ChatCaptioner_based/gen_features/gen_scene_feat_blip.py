...
def gen_points(scene, path, file_list):
    ...
    if len(points) == 0:
        return None
    points = np.stack(points, axis=0)  # N, H, W, 3
    return points
...
for scene in tqdm(scene_list[args.job * ids : min(N, args.job * ids + ids)]):
    ...
    for render_type in ["outside"]:
        ...
        if points is None:
            ...
            continue
        points = points[:, :LOAD_IMG_HEIGHT, :LOAD_IMG_HEIGHT, :]  # N, H, H, 3  NOTE: (H, H)
        points_norm = np.linalg.norm(points, axis=-1)  # N, H, H
        masks = np.logical_and(points_norm > 0, points_norm < 10000)  # N, H, H
        if np.sum(masks) == 0:
        ...
        for i, pf in enumerate(scene_feat_list):
            try:
                features.append(torch.load(os.path.join(scene_feat_path, pf), map_location="cpu").numpy())  # H, H, 1024
            except:
                failed_idx.append(i)
        if len(failed_idx) > 0:
            masks = np.delete(masks, failed_idx, axis=0)
        features = np.stack(features, axis=0)  # N, H, H, F  NOTE: (H, H)
        features = features[masks]  # N, F
        ...
        feature_sum = torch.sum(features, dim=1)  # N, F
