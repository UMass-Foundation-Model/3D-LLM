...
class PNPVQA(BaseModel):
    ...
    def __init__(
        self, image_question_matching_model, image_captioning_model, question_answering_model, offload_model=False
    ):
        super().__init__()
    ...
    def forward_cap(
        ...
        while min_num_captions < num_captions:
            ...
            for i in range(num_captions):
                patch_id = (
                    torch.multinomial(samples["gradcams"].to(self.image_captioning_model.device), num_patches).reshape(
    ...
    def forward_qa(
        ...
        question_captions_chunk = [
            question_captions[i : i + internal_bsz_fid] for i in range(0, len(question_captions), internal_bsz_fid)
        ]
        ...
        for question_caption in question_captions_chunk:
            question_caption_input = self.question_answering_model.tokenizer(
                question_caption, padding="longest", truncation=True, return_tensors="pt"
