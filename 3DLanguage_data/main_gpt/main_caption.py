...
def prepare_prompt(room_dir):
    task_prompt = f"You are a description generator in a room. All object instances in this room are given, along with their center point position.  The center points are represent by a 3D coordinate (x, y, z) with units of meters. You need to describe this room. The description should only be about the room. Do not include objects that do not exist. Do not include exact locations, 3D coordinates, any specific numbers about the positions of the objects!!"
...
def parse_args():
    ...
    parser.add_argument('--room_dir', type=str, default='')

    parser.add_argument('--save_root', type=str, default='./results_caption_final')
    ...
    for i in range(2):
        for room_path in tqdm(room_list):
            try:
                ...
                save_path = os.path.join(args.save_root, f"{room_name}_hm3d_{i}.json")
